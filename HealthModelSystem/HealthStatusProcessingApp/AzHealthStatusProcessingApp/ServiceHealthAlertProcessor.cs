...
{
    public class ServiceHealthAlertProcessor
    {
        private readonly ICosmosSqlRepository _cosmosSqlRepository;
        public ServiceHealthAlertProcessor(ICosmosSqlRepository cosmosSqlRepository)
        {
            _cosmosSqlRepository = cosmosSqlRepository;
        }

        [FunctionName("UpdateServiceHealth")]
        public async Task<IActionResult> Run(
            [HttpTrigger(AuthorizationLevel.Anonymous, "post", Route = null)] HttpRequest req,
            ILogger log)
        {
            log.LogInformation("HTTP trigger function invoked for Service Health Update");

            //TODO: handle exceptions
            string requestBody = await new StreamReader(req.Body).ReadToEndAsync();
            log.LogInformation("Request body:" + requestBody);
            // extract the Resource Health Alert matching the common alert schema from the request body
            ServiceHealthAlert alertData = JsonConvert.DeserializeObject<ServiceHealthAlert>(requestBody);
            // create the data to be persisted
            HealthStatusModel statusData = ServiceHealthMapper.MapServiceHealthToStatusModel(alertData);
